...
export default class ScheduleC extends F1040Attachment {
  ...
  constructor(f1040: F1040, data?: ScheduleCData, index: number = 0) {
    super(f1040)
    this.data = data ?? f1040.info.scheduleCs[0]
    this.index = index
  }
  ...
  started2024 = (): boolean | undefined => this.data.started
  ...
  l4 = (): number | undefined => this.cogs() // Cost of goods sold
  l5 = (): number | undefined => {
    const l3 = this.l3()
    const l4 = this.l4()
  ...
  vehicleDeduction = (): number | undefined => {
    if (this.data.vehicleDeduction && this.data.vehicleDeduction.businessMiles) {
      // 2024 Standard Mileage Rate is 67 cents
      return this.data.vehicleDeduction.businessMiles * 0.67
    }
    return undefined
  }

  l8 = (): number | undefined => this.data.expenses.advertising
  l9 = (): number | undefined => sumFields([this.data.expenses.carAndTruck, this.vehicleDeduction()])
  ...
  l14 = (): number | undefined => this.data.expenses.employeeBenefit
  l15 = (): number | undefined => this.data.expenses.insurance
  l16a = (): number | undefined => this.data.expenses.interestMortgage
  l16b = (): number | undefined => this.data.expenses.interestOther
  ...
  l20a = (): number | undefined => this.data.expenses.rentVehicles
  l20b = (): number | undefined => this.data.expenses.rentOther
  ...
  l35 = (): number | undefined => this.data.costOfGoods?.inventoryBegin
  ...
  l41 = (): number | undefined => this.data.costOfGoods?.inventoryEnd
  ...
  cogs = (): number | undefined => this.l42()

  // Part IV: Vehicle Information
  l43 = (): string | undefined => this.data.vehicleDeduction?.datePlacedInService
  l44a = (): number | undefined => this.data.vehicleDeduction?.businessMiles
  l44b = (): number | undefined => this.data.vehicleDeduction?.commutingMiles
  l44c = (): number | undefined => this.data.vehicleDeduction?.otherMiles
  l45 = (): { select: number } | undefined => this.data.vehicleDeduction?.availableOffDuty ? {select: 0} : {select: 1}
  l46 = (): { select: number } | undefined => this.data.vehicleDeduction?.anotherVehicle ? {select: 0} : {select: 1}
  l47a = (): { select: number } | undefined => this.data.vehicleDeduction?.evidence ? {select: 0} : {select: 1}
  l47b = (): { select: number } | undefined => this.data.vehicleDeduction?.evidenceWritten ? {select: 0} : {select: 1}
